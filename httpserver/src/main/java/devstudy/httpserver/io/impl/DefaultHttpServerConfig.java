...
import devstudy.httpserver.io.exeption.HttpServerConfigException;
...
class DefaultHttpServerConfig implements HttpServerConfig {
	...
	protected BasicDataSource createBasicDataSource() {
		...
		if (Boolean.parseBoolean(serverProperties.getProperty("db.datasource.enabled"))) {
			...
			ds.setInitialSize(Integer
					.parseInt(Objects.requireNonNull(serverProperties.getProperty("db.datasource.pool.initSize"))));
			ds.setMaxTotal(Integer
					.parseInt(Objects.requireNonNull(serverProperties.getProperty("db.datasource.pool.maxSize"))));
