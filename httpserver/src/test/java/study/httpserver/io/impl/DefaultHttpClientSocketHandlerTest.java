...
import study.httpserver.io.config.HttpRequestParser;
...
import study.httpserver.io.exception.BadRequestException;
...
public class DefaultHttpClientSocketHandlerTest {
	...
	public void testHandleHttpServerExceptionDuringProcessRequest() throws IOException {
		doThrow(new BadRequestException("", null, "")).when(httpRequestDispatcher).handle(httpServerContext, request,
				response);
	...
	public void testMethodNotAllowedException() throws IOException {
		when(httpRequestParser.parseHttpRequest(socketInputStream, remoteAddress))
		...
		verify(response).setHeaders("Allow", "GET, POST, HEAD");
	}
	...
	public void testEOFException() throws IOException {
		when(httpRequestParser.parseHttpRequest(socketInputStream, remoteAddress))
				.thenThrow(new EOFException("Inputsream closed"));
