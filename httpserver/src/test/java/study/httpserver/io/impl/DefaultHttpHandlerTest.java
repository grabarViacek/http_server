...
public class DefaultHttpHandlerTest {
	...
	public void testNotFound() throws IOException {
		...
		verify(response, never()).setHeaders(anyString(), any(Object.class));
	}
	...
	public void testFileUri() throws IOException {
		...
		verify(response).setHeaders("Content-Type", "text/css");
		verify(response).setHeaders("Last-Modified",
				Files.getLastModifiedTime(Paths.get(file.toURI()), LinkOption.NOFOLLOW_LINKS));
		verify(response).setHeaders(eq("Expires"), any(Date.class));
	...
	public void testFileWithoutExpiresUri() throws IOException {
		...
		verify(response, never()).setHeaders(eq("Expires"), any(Date.class));
		verify(response, never()).setStatus(anyInt());
		verify(response, never()).setBody(any(Reader.class));
		verify(response, never()).setBody(anyString());
	}
	...
	public void testDirectoryUri() throws IOException {
		...
		when(htmlTemplateManager.processTemplate(eq("simple.html"), anyMapOf(String.class, Object.class)))
				.thenReturn("result");
		Path root = Paths.get(folder.newFolder("root").toURI());
